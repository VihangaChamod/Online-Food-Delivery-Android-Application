...
public class Answers extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        response.setContentType("text/html;charset=UTF-8");
            ...
            String regist_id = request.getParameter("reg_id");
        ...
        try (PrintWriter out = response.getWriter()) {
              ...
              ResultSet guest_details = DBMS_Connection.DBMS.SE("SELECT `id` FROM `guest_details` WHERE `guest_id`='"+regist_id+"'");   

            try {
                if (guest_details.next()) {
                  
                    guest_details_id = guest_details.getString("id");
   ...
   ResultSet cart = DBMS_Connection.DBMS.SE("SELECT `set_id` FROM `cart` WHERE `guest_details_id`='"+guest_details_id+"'");   
